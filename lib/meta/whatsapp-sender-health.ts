...
export function hasMetaWhatsAppRestrictionSignal(sender: MetaWhatsAppSenderLike) {
  ...
  return truthyFlag(metadata.restricted)
    ...
    || lastError.includes('business account locked')
    || lastError.includes('account locked')
    || lastError.includes('conta bloqueada')
    || (lastError.includes('blocked') && (lastError.includes('whatsapp') || lastError.includes('account') || lastError.includes('business')))
