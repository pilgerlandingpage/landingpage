...
export default function AdsPage() {
    ...
    useEffect(() => {
        const fetchReports = async () => {
            try {
                const res = await fetch('/api/admin/reports?platform=google&limit=10')
                if (res.ok) {
                    const data = await res.json()
    ...
    // ── Chart Data: Spend by Campaign (Bar) ──
    ...
    // ── Chart Data: Performance by Campaign (Bar) ──
    ...
    // ── Chart Data: Spend Distribution (Pie) ──
        ...
        <div>
            ...
            <div className="kpi-grid" style={{ gridTemplateColumns: 'repeat(auto-fit, minmax(155px, 1fr))', marginBottom: 24 }}>
                ...
                <div className="kpi-card">
                    ...
                    <div className="kpi-label">CPA Médio</div>
            ...
            {spendBarData.length > 0 && (
                <div style={{ display: 'grid', gridTemplateColumns: '1.5fr 1fr', gap: 24, marginBottom: 24 }}>
                    <div className="chart-card">
                        <div className="chart-title">💰 Gasto & Conversões por Campanha</div>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                                                <div style={{ flex: 1, minWidth: 0 }}>
                                                    ...
                                                    {camp.properties?.title && (
                                                        <div style={{ fontSize: '0.8rem', color: 'var(--text-muted)', marginTop: 2 }}>🏠 {camp.properties.title}</div>
                                                    )}
                                                </div>
                        ...
                        {alerts.length === 0 ? (
                            ...
                            <div style={{ display: 'grid', gap: 8, maxHeight: 600, overflowY: 'auto', paddingRight: 4 }}>
                                {alerts.slice(0, 15).map(alert => {
                                    ...
                                    const actionIcon = alert.action_taken === 'PAUSE_AD' ? '⛔' :
                ...
                <div style={{ display: 'grid', gridTemplateColumns: latestScore != null ? '280px 1fr' : '1fr', gap: 24, marginBottom: 24 }}>
                    ...
                    {latestScore != null && (
                        <div className="chart-card" style={{ textAlign: 'center', padding: '32px 24px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                            ...
                            <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, fontWeight: 600 }}>Termômetro Google Ads</div>
            ...
            <style>{`
                ...
                .report-history-card:hover { border-color: #4285F4; transform: translateX(2px); }
            `}</style>
        </div>
    )
}
